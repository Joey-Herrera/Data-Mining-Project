...
Creating an equitable array of programs to increase parent and family involvement for families of different races, ethnicities, and socioeconomic statuses is a growing issue. In response, the National Household Education Surveys (NHES) program created a national survey with several hundred questions focusing on different aspects of parent and family engagment, This survey is sent to households across the United States, where parents have the opportunity to fill it out and return their responses. The NHES typically recieves around 50 million survey responses each time the survey is sent out. 

This paper seeks to provide basic program recommendations to schools by evaluating the impact of indicators from derived from the NHES parent and family involvement (ppfi) survey on student grades, Using a 1% sample, this paper will use a principal component analysis (PCA) and random forest forest technique to illustrate the effects of different pfi question (or features) on student grades.
...
The following sections of this paper will address the empirical methodology, results, and conclusing remarks associated with the research problem. 
